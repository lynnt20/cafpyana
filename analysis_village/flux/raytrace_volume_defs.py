...
# Full active TPC volume (union of both TPCs). Matches the SBND `volTPCActive`
# from the v02_00 GDML: theTPCActive_W ≈ 201.3 cm per TPC, theTPC_Y ≈ 407.5 cm,
# theTPC_Z ≈ 509.4 cm. Use for closure tests against CAF events cut to active
# volume (matches what GENIEGen sees when TopVolume = volTPCActive).
ACTIVE_TPC_BOXES: list[dict] = [
    dict(x_range=(-201.3, 0.0),   y_range=(-203.7, 203.7), z_range=(0.0, 509.4)),
    dict(x_range=(0.0,    201.3), y_range=(-203.7, 203.7), z_range=(0.0, 509.4)),
]
...
RAYTRACE_VOLUME_LABEL: dict[str, str] = {
    ...
    "FV_split_truncY_eastonly": r"$5<|x|<190$: $|y|<190$ ($10<z<250$); East ($x<0$): $-190<y<100$, West ($x>0$): $|y|<190$ ($250<z<450$)",
}
...
FV_SPLIT_TRUNCY_EAST_VOLUME_CM3: float = (
    2 * 185.0 * 380.0 * 240.0          # both TPCs, low z
    + 185.0 * 290.0 * 200.0            # East, high z, truncated y
    + 185.0 * 380.0 * 200.0            # West, high z, full y
)  # 5.8534e7
ACTIVE_TPC_VOLUME_CM3: float = 2 * 201.3 * 407.4 * 509.4  # ≈ 8.354e7
